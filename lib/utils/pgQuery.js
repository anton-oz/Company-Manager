...
const pgQuery = async (answer, client) => {
    // These variables get reused depending on the case, and hopefully make syntax a bit clearer.
    let department;
    let departments;
    let role;
    let roles;
    ...
    // Switch statement for all inquirer options
    switch (answer) {
        case 'View all departments':
            departments = await client.query('SELECT * FROM department;');
            tableMaker(departments);
            break;
        case 'View all roles':
        ...
        case 'Add new role':
            ...
            await client.query(
                `INSERT INTO role (title, salary, department_id)
                 VALUES
                    ('${role.roleName.trim()}', ${role.salary}, ${departmentId});`);
        ...
        case 'Add new employee':
            ...
            await client.query(
                ...
                VALUES
                    ('${employee.firstName.trim()}', '${employee.lastName.trim()}', ${roleId}, ${managerId});`
            );
...
// Returns matching role_id for role title entered into function. 
...
// Returns array of employees, includes a 'none' option if truthy value passed to second function argument.
...
// Returns matching manager_id for manager name entered into function. 
